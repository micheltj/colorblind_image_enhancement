...
class FrequencyTunedSalientRegionDetection:
    ...
    def transform(self):
        for index, image in enumerate(self.reader.get_images(cv2.COLOR_BGR2RGB)):
            # if index == 1000:
            yield image, get_saliency_ft(img_as_float(image)).astype('uint8')

    def transform_single(self, image):
        return image, get_saliency_ft(img_as_float(image)).astype('uint8')
...
class RobustBackgroundDetection:
    def __init__(self, reader: VideoReader):
        self.reader = reader

    def transform(self):
        for image in self.reader.get_images(cv2.COLOR_BGR2RGB):
            yield image, get_saliency_rbd(image).astype('uint8')
...
def get_saliency_rbd_image(img_path):
...
def get_saliency_ft_img(img_path):
    # Saliency map calculation based on:
