...
class MinimumBarrierSaliency:
    ...
    def transform(self):
        for image in self.reader.get_images(cv.COLOR_BGR2RGB):
            saliency = get_saliency_mbd(img_as_float(image)).astype('uint8')
            yield image, saliency

    def transform_single(self, image):
        return image, get_saliency_mbd(img_as_float(image)).astype('uint8')
...
def get_saliency_mbd_read_image(input, method='b'):
    img_path_list = []
    ...
    for img_path in img_path_list:
        ...
        return get_saliency_mbd(img, method)
